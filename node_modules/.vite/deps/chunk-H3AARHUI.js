...
function containsNumber(coordinates2) {
  ...
  if (Array.isArray(coordinates2[0]) && coordinates2[0].length) {
    return containsNumber(coordinates2[0]);
  }
...
function clipAntimeridianIntersect(lambda04, phi02, lambda12, phi12) {
  ...
  return abs(sinLambda0Lambda1) > epsilon2 ? atan((sin(phi02) * (cosPhi1 = cos(phi12)) * sin(lambda12) - sin(phi12) * (cosPhi03 = cos(phi02)) * sin(lambda04)) / (cosPhi03 * cosPhi1 * sinLambda0Lambda1)) : (phi02 + phi12) / 2;
}
...
function circle_default(radius, delta) {
  ...
  function intersect(a, b, two) {
    ...
    var delta2 = lambda12 - lambda04, polar = abs(delta2 - pi) < epsilon2, meridian = polar || delta2 < epsilon2;
    if (!polar && phi12 < phi02) z = phi02, phi02 = phi12, phi12 = z;
    if (meridian ? polar ? phi02 + phi12 > 0 ^ q[1] < (abs(q[0] - lambda04) < epsilon2 ? phi02 : phi12) : phi02 <= q[1] && q[1] <= phi12 : delta2 > pi ^ (lambda04 <= q[0] && q[0] <= lambda12)) {
...
function resample(project, delta2) {
  function resampleLineTo(x02, y02, lambda04, a0, b0, c0, x12, y12, lambda12, a1, b1, c1, depth, stream) {
    var dx = x12 - x02, dy = y12 - y02, d2 = dx * dx + dy * dy;
    if (d2 > 4 * delta2 && depth--) {
      var a = a0 + a1, b = b0 + b1, c = c0 + c1, m = sqrt(a * a + b * b + c * c), phi2 = asin(c /= m), lambda22 = abs(abs(c) - 1) < epsilon2 || abs(lambda04 - lambda12) < epsilon2 ? (lambda04 + lambda12) / 2 : atan2(b, a), p = project(lambda22, phi2), x2 = p[0], y2 = p[1], dx2 = x2 - x02, dy2 = y2 - y02, dz = dy * dx2 - dx * dy2;
      if (dz * dz / d2 > delta2 || abs((dx * dx2 + dy * dy2) / d2 - 0.5) > 0.3 || a0 * a1 + b0 * b1 + c0 * c1 < cosMinDistance) {
        resampleLineTo(x02, y02, lambda04, a0, b0, c0, x2, y2, lambda22, a /= m, b /= m, c, depth, stream);
        stream.point(x2, y2);
  ...
  return function(stream) {
    var lambda003, x00, y00, a00, b00, c00, lambda04, x02, y02, a0, b0, c0;
    var resampleStream = {
      point: point2,
      lineStart,
      lineEnd,
    ...
    function linePoint2(lambda, phi) {
      ...
      resampleLineTo(x02, y02, lambda04, a0, b0, c0, x02 = p[0], y02 = p[1], lambda04 = lambda, a0 = c[0], b0 = c[1], c0 = c[2], maxDepth, stream);
    ...
    function ringPoint(lambda, phi) {
      linePoint2(lambda003 = lambda, phi), x00 = x02, y00 = y02, a00 = a0, b00 = b0, c00 = c0;
      resampleStream.point = linePoint2;
    }
    function ringEnd() {
      resampleLineTo(x02, y02, lambda04, a0, b0, c0, x00, y00, lambda003, a00, b00, c00, maxDepth, stream);
...
function bounds_default2(feature2) {
  var i, n, a, b, merged, deltaMax, delta;
  phi1 = lambda1 = -(lambda02 = phi0 = Infinity);
  ranges = [];
  stream_default(feature2, boundsStream2);
  if (n = ranges.length) {
...
function containsGeometry(geometry2, point2) {
  return geometry2 && containsGeometryType.hasOwnProperty(geometry2.type) ? containsGeometryType[geometry2.type](geometry2, point2) : false;
}
...
function containsLine(coordinates2, point2) {
  var ab4 = distance_default(coordinates2[0], coordinates2[1]), ao = distance_default(coordinates2[0], point2), ob = distance_default(point2, coordinates2[1]);
  return ao + ob <= ab4 + epsilon2;
}
...
//# sourceMappingURL=chunk-H3AARHUI.js.map
