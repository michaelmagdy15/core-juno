...
const SafeFlightsApp = () => {
  ...
  const [dataSource, setDataSource] = useState<"OPENSKY" | "ADSB_LOL">("OPENSKY");
  ...
  useEffect(() => {
    ...
    const fetchFlights = async () => {
      ...
      try {
        ...
        if (timeOffset < 0) {
          // APIs don't easily do historic radius without auth
          setFlights([]);
          setIsScanning(false);
          return;
        }
        ...
        try {
          ...
          const res2 = await fetch(`https://api.airplanes.live/v2/lat/${centerLat.toFixed(3)}/lon/${centerLon.toFixed(3)}/dist/${Math.ceil(radiusNm)}`);
          if (res2.ok) {
  ...
  const getTerminalSchedule = React.useCallback((airportName: string): any[] => {
    return [];
  }, []);

  return (
    <div className="app-layout flex flex-col h-screen overflow-hidden text-sm md:text-base">
      {/* Top Navigation */}
      <nav className={`top-nav flex flex-col md:flex-row items-center px-4 md:px-6 py-3 border-b gap-3 md:gap-0 ${emergencies.length > 0 ? 'bg-red-950/40 border-red-500/50' : 'bg-zinc-950 border-zinc-800'} transition-colors duration-500 z-50`}>
        <div className="brand flex items-center justify-between w-full md:w-auto md:justify-start gap-3 text-base md:text-lg font-bold">
          <div className="flex items-center gap-3">
            <ShieldAlert className={emergencies.length > 0 ? "text-red-500 animate-pulse" : "text-blue-500"} size={24} />
            <span>SafeFlights <span className="text-zinc-500 font-normal hidden sm:inline">Global Tracker</span></span>
          </div>

          <div className="flex md:hidden items-center gap-2 text-[10px] font-mono">
            <div className="flex items-center gap-1 px-2 py-1 bg-zinc-900 rounded text-zinc-300">
              <Clock size={10} /> {currentTime.toISOString().split('T')[1].substring(0, 5)}
            </div>
            <div className="flex items-center gap-1 px-2 py-1 bg-zinc-900 rounded font-bold" style={{ color: dataSource === 'OPENSKY' ? '#10b981' : dataSource === 'ADSB_LOL' ? '#f97316' : '#ef4444' }}>
              <Activity size={10} /> {dataSource.split('_')[0]}
            </div>
          </div>
        </div>

        <form onSubmit={handleAirportSearch} className="flex-1 w-full md:max-w-lg md:mx-8 relative">
          <div className="flex items-center bg-zinc-900 border border-zinc-700 rounded-md px-3 py-2 text-sm focus-within:border-blue-500 transition-colors">
          ...
          {(searchResults.length > 0 || liveSearchResults.length > 0) && (
            <div className="absolute top-full left-0 right-0 mt-2 bg-zinc-900 border border-zinc-700 rounded-md z-50 shadow-2xl overflow-hidden max-h-[60vh] overflow-y-auto">
              {liveSearchResults.map(f => (
                <div key={f.id} onClick={() => handleFlightSelect(f)} className="px-4 py-3 hover:bg-zinc-800 cursor-pointer flex justify-between items-center border-b border-zinc-800 last:border-0">
                  ...
                  <span className="text-xs text-zinc-400 hidden sm:block">{f.velocity} km/h • {f.alt} ft</span>
                </div>
              ))}
              {searchResults.map((res: any) => (
                <div key={res.place_id} onClick={() => flyToAirportResult(res.lat, res.lon, res.display_name)} className="px-4 py-3 hover:bg-zinc-800 cursor-pointer flex items-center gap-3 border-b border-zinc-800 last:border-0">
                  <MapIcon size={16} className="text-orange-400 shrink-0" /> <span className="truncate flex-1 text-sm">{res.display_name}</span>
                </div>
              ))}
            </div>
          )}
        ...
        <div className="hidden md:flex items-center gap-3 text-xs font-mono">
          ...
          <button onClick={() => setViewMode(viewMode === "2D" ? "3D" : "2D")} className={`flex items-center gap-2 px-3 py-1.5 rounded border transition-colors ${viewMode === "3D" ? 'bg-blue-500/20 border-blue-500/50 text-blue-400' : 'bg-transparent border-zinc-700 text-zinc-400 hover:text-zinc-200'}`}>
            <Globe size={14} /> {viewMode === "2D" ? "3D Globe" : "2D Map"}
          </button>

          <div className="flex items-center gap-2 px-3 py-1.5 bg-zinc-900 rounded text-zinc-300">
            <Clock size={14} /> {currentTime.toISOString().split('T')[1].substring(0, 8)} {timeOffset !== 0 && `(HIST)`}
          </div>
      ...
      <main className="flex-1 flex flex-col md:flex-row overflow-hidden relative">
        {/* Left Side Panel - Hidden completely on mobile if 3d view */}
        <aside className={`${viewMode === '3D' ? 'hidden' : 'flex'} md:flex w-full md:w-80 h-[30vh] md:h-auto bg-zinc-950 border-b md:border-b-0 md:border-r border-zinc-800 flex-col z-40 shrink-0`}>

          {emergencies.length > 0 && (
            <div className="p-2 md:p-4 bg-red-950 border-b border-red-900 overflow-y-auto max-h-[100px] md:max-h-none">
              <div className="text-red-500 font-bold flex items-center gap-2 mb-2 uppercase text-[10px] md:text-xs tracking-wider sticky top-0 bg-red-950">
                <AlertTriangle size={14} className="animate-pulse" /> Emergency Squawk
              </div>
              {emergencies.map(f => (
                <div key={f.id} className="text-xs md:text-sm cursor-pointer hover:bg-red-900/50 p-1.5 rounded transition-colors" onClick={() => handleFlightSelect(f)}>
                  <span className="font-mono font-bold mr-2">{f.num}</span> SQ {f.squawk} at {f.alt} ft
                </div>
              ))}
            </div>
          )}
          ...
          {geofenceAlerts.length > 0 && (
            <div className="p-2 md:p-4 bg-orange-950 border-b border-orange-900 hidden md:block">
              <div className="text-orange-500 font-bold flex items-center gap-2 mb-2 uppercase text-xs tracking-wider">
                <ShieldAlert size={14} /> Restricted Airspace
              </div>
              {geofenceAlerts.slice(0, 3).map((a, i) => (
                <div key={i} className="text-sm text-zinc-300 mb-1">
                  <span className="font-mono font-bold text-white mr-1">{a.flight}</span> entered {a.zone}
                </div>
              ))}
            </div>
          )}
          ...
          {selectedAirportBoard ? (
            <div className="flex-1 flex flex-col p-4 md:p-5 overflow-y-auto">
              <div className="flex items-center justify-between mb-3">
                <div className="text-[10px] md:text-xs uppercase tracking-wider text-zinc-500 font-bold flex items-center gap-2"><MapIcon size={14} /> Terminal Schedule</div>
                <X size={16} className="text-zinc-500 cursor-pointer hover:text-white" onClick={() => setSelectedAirportBoard(null)} />
              </div>
              <h2 className="text-lg md:text-xl font-bold text-white mb-4">{selectedAirportBoard.name}</h2>
              <div className="flex flex-col gap-3">
                <div className="text-center text-xs text-zinc-500 mt-4 italic">Live schedule data integration pending...</div>
              </div>
            </div>
          ) : (
            <div className="flex-1 flex flex-col p-3 md:p-4 overflow-y-auto">
              <div className="text-[10px] md:text-xs uppercase tracking-wider text-zinc-500 font-bold flex items-center gap-2 mb-2 md:mb-4 pb-2 border-b border-zinc-800 sticky top-0 bg-zinc-950">
                <Plane size={14} /> Active Radar Feed ({flights.length})
              </div>
              <div className="grid grid-cols-2 md:flex md:flex-col gap-2 md:gap-3">
                {flights.length === 0 ? (
                  <div className="text-center text-zinc-600 my-4 md:my-10 col-span-2"><Activity size={24} className="mx-auto mb-2 opacity-50" /> {isScanning ? 'Scanning...' : 'No Aircraft'}</div>
                ) : (
                  flights.slice(0, 50).map(flight => {
                    ...
                    return (
                      <div key={flight.id} onClick={() => handleFlightSelect(flight)} className={`p-2 md:p-4 rounded-lg border ${bColor} cursor-pointer transition-all`}>
                        <div className="flex justify-between items-start mb-2">
                          <div>
                            <div className="font-mono font-bold text-white text-sm md:text-lg leading-none mb-1">{flight.num}</div>
                            <div className="text-[9px] md:text-xs text-zinc-500 uppercase tracking-widest">{flight.country}</div>
                          </div>
                          <span className={`hidden md:inline-block text-[9px] md:text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded ${flight.onGround ? 'bg-orange-500/20 text-orange-400' : 'bg-zinc-800 text-zinc-300'}`}>
                            {flight.onGround ? 'GND' : 'AIR'}
                          </span>
                        </div>
                        <div className="flex flex-col md:grid md:grid-cols-2 gap-1 md:gap-2 text-[10px] md:text-xs font-mono">
                          <div><span className="text-zinc-600 mr-2">ALT</span><span className="text-green-400">{flight.alt}'</span></div>
                          <div><span className="text-zinc-600 mr-2">SPD</span><span className="text-zinc-300">{flight.velocity}</span></div>
                        </div>
                      </div>
                    )
                  })
        ...
        {/* Mobile quick actions bar (Shows in 3D Mode or Map) */}
        <div className="md:hidden absolute top-4 right-4 z-[9999] flex flex-col gap-2">
          <button onClick={() => setViewMode(viewMode === "2D" ? "3D" : "2D")} className="bg-zinc-900/80 backdrop-blur border border-zinc-700 p-2.5 rounded-full shadow-lg text-white">
            <Globe size={18} className={viewMode === '3D' ? "text-blue-400" : ""} />
          </button>
          {viewMode === '2D' && (
            <button onClick={() => setIsHeatmap(!isHeatmap)} className="bg-zinc-900/80 backdrop-blur border border-zinc-700 p-2.5 rounded-full shadow-lg text-white">
              <ThermometerSun size={18} className={isHeatmap ? "text-orange-400" : ""} />
            </button>
          )}
        </div>

        {/* Cinematic Map / 3D Canvas area */}
        <section className="flex-1 relative bg-[#09090b] h-full z-0 overflow-hidden">
          {viewMode === "3D" ? (
            <div className="absolute inset-0 z-10 fade-in pointer-events-auto flex flex-col">
              <div className="absolute top-4 left-4 z-20 bg-black/50 backdrop-blur-md p-3 md:p-4 rounded-lg border border-zinc-800 text-white max-w-[200px] md:max-w-sm pointer-events-none">
                <h3 className="font-bold text-sm md:text-lg mb-1 md:mb-2 flex items-center gap-2"><Globe className="text-blue-400" size={16} /> Immersive 3D</h3>
                <p className="text-[10px] md:text-sm text-zinc-400 hidden md:block">Interact with the globe to explore the situational theater in full 3D space utilizing the Spline runtime engine. Drag to rotate, scroll to zoom.</p>
              </div>
              ...
              {flights.map(flight => (
                <Marker key={flight.id} position={[flight.lat, flight.lon]} icon={createPlaneIcon(flight.heading, flight.id === selectedFlight?.id, flight.alt, isHeatmap, flight.squawk)} eventHandlers={{ click: () => handleFlightSelect(flight) }}>
                  <Popup className="tactical-popup" closeButton={false}>
                    <div className="bg-zinc-950 border border-zinc-800 rounded-lg p-3 md:p-4 shadow-2xl min-w-[200px] md:min-w-[260px]">
                      <div className="flex gap-3 md:gap-4 items-center border-b border-zinc-800 pb-3 md:pb-4 mb-3 md:mb-4">
                        <div className="w-10 h-10 md:w-12 md:h-12 bg-white rounded flex items-center justify-center p-1 overflow-hidden shrink-0">
                        ...
                        <div>
                          <div className="text-lg md:text-xl font-bold font-mono text-white leading-none mb-1">{flight.num}</div>
                          <div className="text-[9px] md:text-[10px] text-zinc-400 font-bold uppercase tracking-wider line-clamp-1">{selectedFlightDetails?.Type || 'AIRCRAFT UNKNOWN'}</div>
                          <div className="text-[9px] md:text-[10px] text-zinc-500 font-mono mt-1">REG: {selectedFlightDetails?.Registration || flight.id.toUpperCase()}</div>
                        </div>
                      </div>
                      <div className="grid grid-cols-2 gap-y-3 gap-x-2 text-[10px] md:text-xs font-mono">
                        <div><div className="text-[8px] md:text-[9px] text-zinc-600 uppercase mb-0.5 md:mb-1">True Track</div><div className="text-zinc-200 flex items-center gap-1"><Compass size={10} />{flight.heading}°</div></div>
                        <div><div className="text-[8px] md:text-[9px] text-zinc-600 uppercase mb-0.5 md:mb-1">Gnd Speed</div><div className="text-zinc-200">{flight.velocity} km/h</div></div>
                        <div><div className="text-[8px] md:text-[9px] text-zinc-600 uppercase mb-0.5 md:mb-1">Baro Alt</div><div className="text-green-400">{flight.alt} ft</div></div>
                        <div><div className="text-[8px] md:text-[9px] text-zinc-600 uppercase mb-0.5 md:mb-1">Squawk</div><div className={flight.squawk === '7700' ? "text-red-500 font-bold animate-pulse" : "text-zinc-400"}>{flight.squawk || 'NONE'}</div></div>
                      </div>
                    </div>
                  </Popup>
                </Marker>
          ...
          <div className="absolute bottom-0 left-0 right-0 p-3 md:p-6 bg-gradient-to-t from-black/90 to-transparent z-40 pointer-events-none">
            <div className="max-w-4xl mx-auto bg-zinc-950/90 backdrop-blur border border-zinc-800 rounded-lg md:rounded-xl p-3 md:p-4 flex flex-col gap-2 md:gap-3 shadow-2xl pointer-events-auto">
              <div className="flex justify-between items-center text-[10px] md:text-xs font-bold uppercase tracking-wider">
                <span className="text-zinc-400 flex items-center gap-2"><History size={12} className="md:w-3.5 md:h-3.5" /> Historical Replay</span>
                <span className={timeOffset === 0 ? "text-green-400" : "text-orange-400"}>
                  {timeOffset === 0 ? "LIVE" : `${Math.abs(timeOffset)}HR AGO`}
                </span>
              </div>
              <input
                ...
                className="w-full accent-blue-500 h-1.5 md:h-auto"
              />
              <div className="flex justify-between text-[8px] md:text-[10px] text-zinc-600 font-mono font-bold">
                <span>-24H</span><span className="hidden md:inline">-18H</span><span>-12H</span><span className="hidden md:inline">-6H</span><span>NOW</span>
              </div>
            </div>
          </div>
        </section>
